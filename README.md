...
## Введение
Данный раздел демонстрирует применение принципов Clean Code к рефакторингу плохо структурированного кода. В примере показано преобразование непонятного кода в читаемый, документированный и поддерживаемый.
...
- Непонятные имена переменных и методов (`d`, `x`, `i`, `j`)
- Слишком большой метод, выполняющий несколько задач
- Отсутствие документации
- Дублирование логики
- Отсутствие обработки ошибок
- Сложные вложенные циклы без пояснений

---

### Рефакторинг согласно принципам Clean Code

#### 1. Переименование переменных и методов
- Метод `d` → `PrintIdentityMatrix`
- Переменная `x` → `size`
- Циклы с `i`, `j` → `row`, `column`

#### 2. Разбиение на маленькие методы
- Выделена отдельная проверка размера матрицы
- Создан метод для вывода одной строки матрицы

#### 3. Добавление XML-документации
- Добавлены комментарии ко всем public методам

#### 4. Удаление дублирующегося кода
- Логика вывода матрицы вынесена в отдельный метод

#### 5. Обработка ошибок
- Добавлена валидация входных данных

#### 6. Комментарии только для сложной бизнес-логики
- Убраны избыточные комментарии

---

### Требуемый результат (чистый код)
...
/// <summary>
/// Валидирует размер матрицы
/// </summary>
/// <param name="size">Размер матрицы</param>
/// <exception cref="ArgumentException">Если размер меньше или равен 0</exception>
private void ValidateMatrixSize(int size)
{
    if (size <= 0)
    {
        throw new ArgumentException("Размер матрицы должен быть положительным числом", nameof(size));
    }
}

/// <summary>
/// Выводит одну строку единичной матрицы
/// </summary>
/// <param name="row">Номер текущей строки</param>
/// <param name="size">Размер матрицы</param>
private void PrintMatrixRow(int row, int size)
{
    for (int column = 0; column < size; column++)
    {
        Console.Write(column == row ? "1 " : "0 ");
    }
    Console.WriteLine();
}
```
...
#### 1. Понятные имена
- `PrintIdentityMatrix` вместо `d` — сразу понятно назначение метода
- `size`, `row`, `column` вместо `x`, `i`, `j` — описывают реальную сущность

#### 2. Маленькие методы
- Каждый метод выполняет **одну задачу**:
  - `PrintIdentityMatrix` — координирует процесс
  - `ValidateMatrixSize` — проверяет входные данные
  - `PrintMatrixRow` — выводит одну строку матрицы

#### 3. XML-документация
- Все public методы имеют подробные комментарии
- Указаны параметры и возможные исключения

#### 4. Удаление дублирования
- Логика вывода матрицы вынесена в отдельный метод
- Упрощена структура циклов

#### 5. Обработка ошибок
- Добавлена проверка на корректность входных данных
- Более информативное исключение с указанием параметра

#### 6. Читаемость
- Улучшена структура кода
- Убраны избыточные комментарии
- Добавлены пробелы для лучшего восприятия
...
Все переменные имеют понятные имена, отражающие их назначение.

✅ **Правильность разбиения на методы**  
Каждый метод отвечает за одну задачу, соблюдается принцип единой ответственности.

✅ **Наличие документации**  
Все public методы имеют полную XML-документацию с описанием параметров и исключений.

✅ **Удаление дублирования**  
Логика вывода матрицы вынесена в отдельный метод.

✅ **Улучшение читаемости**  
Код структурирован, имеет правильные отступы и пробелы, легко воспринимается.

---

### Заключение
Рефакторинг позволил превратить непонятный, трудно поддерживаемый код в чистый, документированный и легко читаемый. Такой код соответствует стандартам Clean Code и значительно упрощает будущее развитие проекта.
